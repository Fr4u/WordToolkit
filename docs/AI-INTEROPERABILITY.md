...
- `source.native_action_contract_sha256`: canonical native 88-action subset, core
...
This keeps discovery bounded instead of paying for all 88 schemas.
...
All 88 operations have input schemas and MCP effect annotations. Semantic query,
semantic HTML rendering, semantic-style plan/apply and comment-body plan/apply have
explicit output-schema, permission, reversibility and per-operation-version metadata;
the remaining 82 are still uncovered. Missing metadata
