...
- Catalog-backed Word member batch diagnostics: fake-COM tests run a three-operation mutating batch, fail the second operation, prove the third never executes, verify rollback/version preservation and require zero-based `failed_operation_index=1`. A separate Undo-finalization failure proves transaction-wide errors omit a false index and return `failed_operation_index_available=false` with `failure_scope="batch"`.
