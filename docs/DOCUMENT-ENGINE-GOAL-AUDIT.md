...
| Capability negotiation | Partial (versioned discovery slice strengthened) | `get_wordtoolkit_capabilities` and the native `capabilities` CLI return one schema-versioned, hashed, paged contract for 85 actions without opening Word; semantic query is the first action with an explicit operation version, output schema, normalized permission and reversibility records in its inspected action contract, while the closed v1 summary keeps its original shape; Word COM member capabilities remain separate | Migrate the remaining 84 actions and add a unified backend/document/feature matrix with runtime probes |
| Public SDK/CLI/MCP operation convergence | Partial (third vertical slice) | `wordtoolkit.inspect_ooxml_package/1.0`, `wordtoolkit.transform_ooxml_package/1.0` and `wordtoolkit.query_ooxml_semantics/1.0` own typed request/result/error/JSON contracts in `WordToolkit.Engine`. Query now shares one high-level, privacy-bounded result across direct and indexed SDK, non-interactive JSON CLI and MCP execution; transform provides replace-first-text, accept-all and reject-all with canonical parity, candidate validation, signature/output-collision gates and no Word dependency and also implements the neutral protocol-v1 benchmark adapter | Migrate remaining operations behind the same application boundary; add explicit output schemas and language packages generated from canonical contracts |
...
| Requirement | State | Current evidence | Exit condition |
|---|---|---|---|
| Unit/regression tests | Partial | 366 engine, 247 native and 1273 Python tests pass at the current checkpoint, with 16 intentional Python skips | Coverage for every required feature and published failure corpus |
