...
| Capability negotiation | Partial (versioned discovery slice implemented) | `get_wordtoolkit_capabilities` and the native `capabilities` CLI return one schema-versioned, hashed, paged contract for 85 actions without opening Word; Word COM member capabilities remain separate | Add normalized permissions/reversibility/output schemas and a unified backend/document/feature matrix with runtime probes |
| Public SDK/CLI/MCP operation convergence | Partial (second vertical slice) | `wordtoolkit.inspect_ooxml_package/1.0` and `wordtoolkit.transform_ooxml_package/1.0` own typed request/result/error/JSON contracts in `WordToolkit.Engine`. The second slice provides replace-first-text, accept-all and reject-all package transforms through SDK, non-interactive CLI and MCP with canonical parity, candidate validation, signature/output-collision gates and no Word dependency; the same core also implements the neutral protocol-v1 benchmark adapter | Migrate remaining operations behind the same application boundary; add explicit output schemas and language packages generated from canonical contracts |
...
| Release packaging | Partial (strengthened) | The 0.35 development package is self-contained Windows x64, contains the engine/runtime/manifest and zero Python files. Packaging canonicalizes copied JSON/Markdown and the embedded schema to BOM-less UTF-8/LF and normalizes Windows packaging onto Windows PowerShell 5.1. The current transform checkpoint produced the same 195-file, 85,150,302-byte tree and 36,200,235-byte ZIP in two local builds at SHA-256 `e1ef21c763cae801f48bcfa43d1513ff279936a8fd026e00b28a04133755afe8`; packaged discovery reported 85 actions/15 exposed tools and opened no Word process. Hosted proof is not claimed until CI returns. The preceding inspection-operation checkpoint matched mandatory CI run `29967113037` with identical local/hosted 195-file, 85,107,806-byte trees and 36,189,483-byte ZIPs at SHA-256 `ff910c0c314ccb98b7f716f40cfa6e4580659b763ee3d657e138c1d6732b4632`. The earlier exact 0.35 live package, SHA-256 `e8f2e4b74fe65213197126c7aafb445452bd0e80bc05f7206d82672e4b09e59b`, passed the complete 48-action real-Word gate; later saved-package slices do not change live COM code | Optional signing/provenance policy, published artifact, exact hosted transform artifact and refreshed licensed Word gate before release |

## Current checkpoint evidence

- `dotnet test native/WordToolkit.Engine.Tests` — 366 passed.
- `dotnet test native/WordToolkit.Native.Tests` — 247 passed.
...
- Two current local package builds are byte-identical: 195 files, 85,150,302 expanded
  ...
  transform action and leaves the Word-process count at zero. Hosted artifact parity
  remains unclaimed until the pushed commit's mandatory CI completes.
- The current development head exposes 15 public tools and 85 lazy actions. The
  transform operation is discoverable in the same vendor-neutral manifest and uses one
  typed core contract across SDK, CLI, MCP and the public benchmark protocol.
