...
| Requirement | State | Current evidence | Exit condition |
|---|---|---|---|
| Unit/regression tests | Partial | 331 engine, 223 native and 1273 Python tests pass at the current checkpoint, with 16 intentional Python skips | Coverage for every required feature and published failure corpus |
...
- `dotnet test native/WordToolkit.Engine.Tests` — 331 passed.
- `dotnet test native/WordToolkit.Native.Tests` — 223 passed.
...
- `scripts/build_native_plugin.ps1` — self-contained native package built with no
  Python runtime.
- The current development head exposes 14 public tools and 83 lazy actions. Two local
  builds produced the same 195-file, 84,897,393-byte tree and 36,123,774-byte ZIP with
  SHA-256
  `dcaa12c58eed3b1b03f10c6772083a934c62e84f4615e738bee975f37fc7d471`.
  Through the packaged executable, the advanced torture DOCX produced one control, one
  resolved binding, one target and two stores in a 2,718-character complete response.
  It opened no Word process, returned no source part path or raw XML and used no Python.
