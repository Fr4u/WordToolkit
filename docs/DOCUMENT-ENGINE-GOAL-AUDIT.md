...
| AI planner | Partial foundation (strengthened) | Deterministic bounded text plans and heterogeneous create/clone/exact-consolidate/proven-unused-delete/assign style plans, typed review-decision plans, exact package-patch plans and three-way merge plans report counts/impact without returning content. Style plans can create or clone a definition, strictly consolidate an explicit equivalent custom source, delete an explicitly selected custom non-default definition only after complete consumer proof, and server-select assignment targets in one reviewed atomic intent. Merge exposes stable conflict IDs, explicit three-choice resolutions, independently authorized risk classes, exact identities and baseline/candidate validation; lazy stateless apply requires reviewed IDs and all source fingerprints | Natural-language intent -> evidence -> broader heterogeneous typed plan -> cost/risk -> richer permissions and approval policy |
| Typed semantic mutations | Partial (text + style-definition/assignment + review transaction slices) | Text plans edit source-bound `w:t`, `w:delText` and `m:t`; `create_style` emits a minimal typed inherited definition, `clone_style` preserves an existing definition's modeled and opaque formatting under safe custom identity, `consolidate_style` rewrites modeled references and removes an exactly equivalent custom source, `delete_unused_style` removes only explicitly selected custom non-default definitions with no surviving modeled, glossary, latent, field or unmodeled consumer, and exact `set_style`/token-lean `set_style_where` assign existing or same-plan-created compatible styles. One plan composes sequential `styles.xml`, numbering and selected story changes, validates intermediate graphs and the final candidate, predicts one fingerprint and retains an exact inverse. Review plans accept/reject supported insertion/deletion/conflict wrappers, complete moves and property snapshots, plus numbering-change acceptance, inserted rows, cell-insertion acceptance and cell-deletion rejection. Unsupported or ambiguous structures fail closed | Arbitrary typed style formatting, rename/referenced-or-built-in deletion/fuzzy repair/template alignment and `stylesWithEffects` mirroring; broader paragraph/run/table/field/math commands, remaining review vocabulary, permissions and durable recovery artifacts |
...
| `styles.xml` | Partial (typed read graph + create/clone/exact-consolidate/proven-unused-delete/assignment mutation) | Defaults, latent metadata, four style types, inheritance diagnostics, theme integration and modeled effective formatting exist. Typed commands insert minimal inherited custom definitions, clone existing definitions with opaque extension preservation, remove an explicit exactly equivalent custom source after modeled reference rewrites, or delete an explicit custom non-default definition after complete consumer proof, then assign styles in the same atomic transaction without reserializing unrelated bytes | Arbitrary property authoring, safe rename/referenced-or-built-in deletion/fuzzy repair, automatic linked-style maintenance, `stylesWithEffects` mirroring, conditional table behavior and template alignment |
