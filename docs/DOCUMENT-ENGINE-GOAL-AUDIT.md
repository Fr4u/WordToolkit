...
| Semantic HTML/SVG preview | Partial (HTML plus fingerprint-bound object selection) | `wordtoolkit.render_ooxml_semantic_html/1.0` projects the main body, all text stories or exactly one stable `target_node_id` to deterministic self-contained HTML through one Engine/CLI/MCP path. Selection requires the inspected package fingerprint, fails closed for stale/missing/out-of-scope/non-renderable targets, renders no siblings and recursively synthesizes valid table context around selected/nested tables, rows, cells, row-level revisions and arbitrarily nested pure SDT wrapper chains; ambiguous mixed chains fail closed. Links remain inert, external resources and active content are never loaded, fields suppress instructions, revisions are annotated, equations are bounded linear-text fallbacks, unsupported drawings/extensions are visible placeholders, output is create-new and bounded to 256 MiB, and the result explicitly reports both `semantic_preview_non_paginated` and that the local artifact contains document content while the MCP response does not | Add SVG geometry, richer accessible math/list semantics, CSS/effective-format coverage, browser snapshots, selection-to-PNG/PDF backends and a shared cross-backend visual corpus without weakening the non-authoritative label |
...
| Capability negotiation | Partial (versioned discovery slice strengthened) | `get_wordtoolkit_capabilities` and the native `capabilities` CLI return one schema-versioned, hashed, paged contract for 88 actions without opening Word; semantic query, semantic HTML rendering, semantic-style plan/apply and comment-body plan/apply have explicit operation versions, output schemas, normalized permission and reversibility records in their inspected action contracts, while the closed v1 summary keeps its original shape; Word COM member capabilities remain separate | Migrate the remaining 82 actions and add a unified backend/document/feature matrix with runtime probes |
| Public SDK/CLI/MCP operation convergence | Partial (sixth vertical slice) | Inspection, transform, semantic query, semantic HTML rendering, semantic-style plan/apply and comment-body plan/apply own typed request/result/error contracts in `WordToolkit.Engine`. The renderer shares one strict request parser and one deterministic create-new implementation across direct .NET, non-interactive JSON CLI and lazy MCP without opening Word or returning document text in the response. Comment edits retain their invariant-checked, schema-validated atomic path. Transform still provides replace-first-text, accept-all and reject-all and implements the neutral protocol-v1 benchmark adapter | Migrate remaining operations behind the same application boundary; generate language packages from canonical contracts and add a public rollback action rather than treating the backup path as a complete transaction API |
...
| render to HTML | Partial: whole main body or all projected text stories render to deterministic, self-contained, non-paginated semantic HTML with inert links, revision annotations, field-instruction suppression, linear equation fallbacks, explicit placeholders and an atomic create-new artifact contract; page/layout fidelity, object selection and richer formatting remain |
...
| Requirement | State | Current evidence | Exit condition |
|---|---|---|---|
| Unit/regression tests | Partial | 396 engine, 269 native and 1276 Python tests pass at the current checkpoint, with 16 intentional Python skips | Coverage for every required feature and published failure corpus |
...
| Visual regression | Partial static semantic-HTML checkpoint | Packaged whole-document HTML parses into the expected story/paragraph tree, includes CSP and contains no external resource elements. The packaged selected Mammoth table parses as one table/one body/two rows/four cells with zero invalid table/body/row/cell parents. The in-app browser blocked the local `file://` URL, so no browser screenshot or visual claim was fabricated | Versioned browser snapshots plus PDF/page/object baselines across rendering backends |
...
| Release packaging | Partial (strengthened) | The 0.35 development package is self-contained Windows x64, contains the engine/runtime/manifest and zero Python files. Packaging canonicalizes copied JSON/Markdown and the embedded schema to BOM-less UTF-8/LF and normalizes Windows packaging onto Windows PowerShell 5.1. The fingerprint-bound selection checkpoint produced identical 196-file, 85,442,968-byte trees and 36,285,575-byte ZIPs in two local builds at SHA-256 `a0737809d981cd739cade8f4036818408877afd903ff1049d538f29b4b99fb41`; explicitly defined UTF-8/LF manifests of sorted relative path, byte length and file SHA-256 fields matched at SHA-256 `a45a778680bf0c9773089556e8a88104d155ebbf044bd852a24f20155d0596e4`. Hosted proof for this exact fingerprint is pending the pushed selection commit; run `29984588930` remains the completed proof for the preceding whole-document renderer checkpoint. The earlier exact 0.35 live package, SHA-256 `e8f2e4b74fe65213197126c7aafb445452bd0e80bc05f7206d82672e4b09e59b`, passed the complete 48-action real-Word gate; later saved-package slices do not change live COM code | Optional signing/provenance policy and refreshed licensed Word gate before release |

## Current checkpoint evidence

- `dotnet test native/WordToolkit.Engine.Tests` — 396 passed.
- `dotnet test native/WordToolkit.Native.Tests` — 269 passed.
- `.venv/Scripts/python -m pytest -q` — 1276 passed, 16 intentionally skipped.
- The current semantic-HTML checkpoint exposes
  `wordtoolkit.render_ooxml_semantic_html/1.0` through one provider-neutral Engine path
  shared by direct .NET, strict JSON CLI and lazy MCP. It creates a bounded, deterministic,
  self-contained artifact without opening Word, loading external relationships or
  executing active content; the fidelity class remains explicitly non-paginated. Two
  supported package builds matched exactly at 196 files, 85,442,968 expanded bytes and
  ZIP SHA-256 `a0737809d981cd739cade8f4036818408877afd903ff1049d538f29b4b99fb41`;
  expanded manifests matched at SHA-256
  `a45a778680bf0c9773089556e8a88104d155ebbf044bd852a24f20155d0596e4`.
  Packaged rendering of the real Mammoth fixture produced the same 3,628-byte artifact
  across 15 cold processes at 254.66 ms median and 290.44 ms p95/max, left the fixture
  tracked-clean, returned no document text or XML in the response and passed static
  HTML-tree, CSP and external-resource checks. Browser visual proof remains unperformed
  because the in-app browser blocked the local URL and no bypass was attempted. A packaged
  hostile compression-ratio fixture proves that package-derived entry names do not enter
  CLI errors and that no output is created.
