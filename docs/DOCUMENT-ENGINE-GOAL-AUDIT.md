...
| Document dependency graph | Partial (strengthened cross-domain spine) | Deterministic `wddn_` nodes and `wdde_` edges join OPC reachability, semantic containment, explicit paragraph/run/table styles, style inheritance/link/defaults, numbering instances/abstracts/levels/picture bullets, field/bookmark targets, section header/footer bindings, classic charts, content controls/Custom XML bindings, nested tables and vertical-merge continuation cells. Missing and external targets remain explicit; every endpoint, input fingerprint and resource budget is checked. Lazy inspection redacts keys/source by default and provides bounded impact traversal | DrawingML/VML layout, SmartArt, OLE, citations/bibliography sources, macros/signatures/encryption, co-authoring sessions, semantic query joins, incremental invalidation and mutation-impact policies |
...
| generate a dependency graph | Partial: one bounded graph now joins OPC, semantic containment, styles, numbering, references, sections, classic charts, content controls, physical/built-in XML stores, binding targets, repeating-section items, nested tables and vertical-merge continuation cells with explicit coverage gaps; drawings, embedded objects, bibliography, active content and collaboration remain outside the typed spine |
...
| Release packaging | Partial (strengthened) | The 0.36 development package is self-contained Windows x64, contains the engine/runtime/manifest and zero Python files. Packaging canonicalizes copied JSON/Markdown and the embedded schema to BOM-less UTF-8/LF and normalizes Windows packaging onto Windows PowerShell 5.1. The exact-target SVG checkpoint produced identical 196-file, 85,514,480-byte trees and 36,306,172-byte ZIPs in two repository-pinned .NET SDK 8.0.423 local builds and hosted artifact `8557592140` at SHA-256 `5076fc4b41670ae05752ab36c0d23ff66fac7a2b8d753706b15473a478e4682a`; explicitly defined UTF-8/LF manifests of sorted relative path, byte length and file SHA-256 fields matched the hosted tree and enabled personal-plugin cache at SHA-256 `f9bfd22a28d04e509968f3fb799e2e283e9ff8e3ccabdb14fdbc7503c1bc7207` with zero file differences. Hosted CI run `29992366785` passed all five jobs for commit `d6b663922a45f8e271872f61a63dffa86bf21256`. An initial SDK 10.0.300 package was rejected as release evidence after its self-contained runtime differed from the pinned-SDK output. The preceding 0.35 HTML checkpoint passed all five jobs in run `29988472344`, and its downloaded Windows artifact matched its two local builds byte for byte. The earlier exact 0.35 live package, SHA-256 `e8f2e4b74fe65213197126c7aafb445452bd0e80bc05f7206d82672e4b09e59b`, passed the complete 48-action real-Word gate; later saved-package slices do not change live COM code | Optional signing/provenance policy and refreshed licensed Word gate before release |

## Current checkpoint evidence

- `dotnet test native/WordToolkit.Engine.Tests` — 408 passed.
- `dotnet test native/WordToolkit.Native.Tests` — 272 passed.
- `.venv/Scripts/python -m pytest -q` — 1279 passed, 16 intentionally skipped.
- The current semantic-SVG checkpoint exposes
  `wordtoolkit.render_ooxml_semantic_svg/1.0` through the same provider-neutral package,
  exact-target and create-new boundary used by Engine, strict JSON CLI and lazy MCP. The
  artifact is deterministic, self-contained and explicitly non-paginated with estimated
  text metrics and no pixel-equivalence claim. Two supported package builds matched
  exactly at 196 files, 85,514,480 expanded bytes and ZIP SHA-256
  `5076fc4b41670ae05752ab36c0d23ff66fac7a2b8d753706b15473a478e4682a`;
  expanded and enabled-plugin manifests matched at SHA-256
  `f9bfd22a28d04e509968f3fb799e2e283e9ff8e3ccabdb14fdbc7503c1bc7207`.
  Hosted run `29992366785` passed all five jobs; its artifact `8557592140` matched both
  local pinned-SDK ZIPs and expanded trees exactly. The earlier SDK 10.0.300 build was
  rejected as release evidence when its bundled self-contained runtime differed.
  Packaged rendering of the real Mammoth table produced a 2,266-byte SVG with real text,
  no script/`foreignObject`/active-link/event/external-URI nodes or attributes, no source
  mutation, no Word process and no document text or local path in the response. The
  10,000-node benchmark emitted the same 1,305 bytes and SHA-256 seven times at 449.10 ms
  median and 844.87 ms p95/max; whole-package projection still dominates. Security tests
  additionally reject UNC/device namespaces before filesystem I/O and stop resource-heavy
  SVG construction before final XML materialization.
