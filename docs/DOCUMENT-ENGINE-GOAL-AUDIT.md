...
| AI planner | Partial foundation (strengthened) | Deterministic bounded text plans and heterogeneous create/clone/exact-consolidate/assign style plans, typed review-decision plans, exact package-patch plans and three-way merge plans report counts/impact without returning content. Style plans can create or clone a definition, strictly consolidate an explicit equivalent custom source, and server-select assignment targets in one reviewed atomic intent. Merge exposes stable conflict IDs, explicit three-choice resolutions, independently authorized risk classes, exact identities and baseline/candidate validation; lazy stateless apply requires reviewed IDs and all source fingerprints | Natural-language intent -> evidence -> broader heterogeneous typed plan -> cost/risk -> richer permissions and approval policy |
| Typed semantic mutations | Partial (text + style-definition/assignment + review transaction slices) | Text plans edit source-bound `w:t`, `w:delText` and `m:t`; `create_style` emits a minimal typed inherited definition, `clone_style` preserves an existing definition's modeled and opaque formatting under safe custom identity, `consolidate_style` rewrites modeled references and removes an exactly equivalent custom source, and exact `set_style`/token-lean `set_style_where` assign existing or same-plan-created compatible styles. One plan composes sequential `styles.xml`, numbering and selected story changes, validates intermediate graphs and the final candidate, predicts one fingerprint and retains an exact inverse. Review plans accept/reject supported insertion/deletion/conflict wrappers, complete moves and property snapshots, plus numbering-change acceptance, inserted rows, cell-insertion acceptance and cell-deletion rejection. Unsupported or ambiguous structures fail closed | Arbitrary typed style formatting, rename/general deletion/fuzzy repair/template alignment and `stylesWithEffects` mirroring; broader paragraph/run/table/field/math commands, remaining review vocabulary, permissions and durable recovery artifacts |
...
| Optimizer | Partial (first strict style slice) | Explicit `consolidate_style` removes one or more proven canonical-equivalent custom definitions only after type-checking and rewriting recognized story/style/numbering references; exact inverse and candidate validation preserve rollback and structural proof | Lint-driven candidate selection, dead-part/image/font optimization, ZIP recompression policy, broader style minimization and measured size/fidelity corpus |
| Repair engine | Partial (strengthened fail-closed slices) | `WordLintRepairPlanner` accepts only a package-bound empty-title finding with exactly one existing, empty, leaf `dc:title`. Separately, the semantic transaction planner can repair an explicitly chosen exact duplicate custom style by consolidating it into a same-type target and blocks uncertain consumers. Both paths verify fingerprints, perform lossless edits, predict result fingerprints and retain exact inverses; lazy plan/apply adds digital-signature blocking and baseline-aware Open XML validation without returning XML | Missing-title part creation, linter-to-style-repair binding, fuzzy or ranked style repair, broader dependency-aware repair sets, richer authorization/recovery, Word round-trip and corpus proof |
...
| `styles.xml` | Partial (typed read graph + create/clone/exact-consolidate/assignment mutation) | Defaults, latent metadata, four style types, inheritance diagnostics, theme integration and modeled effective formatting exist. Typed commands insert minimal inherited custom definitions, clone existing definitions with opaque extension preservation, or remove an explicit exactly equivalent custom source after modeled reference rewrites, then assign styles in the same atomic transaction without reserializing unrelated bytes | Arbitrary property authoring, safe rename/general deletion/fuzzy repair, automatic linked-style maintenance, `stylesWithEffects` mirroring, conditional table behavior and template alignment |
...
| repair styles | Partial: explicit `consolidate_style` repairs only a proven canonical-equivalent custom duplicate under exact source/target IDs, complete modeled reference rewrites, strict consumer blockers, one predicted fingerprint, SDK candidate validation and exact inverse; fuzzy equivalence, automatic ranking/linter binding, rename, general deletion and template alignment remain |
...
| minimize package size | Partial: exact custom-style consolidation removes redundant definitions and rewrites their modeled consumers with preservation proof; dead parts, images, fonts, ZIP policy and automatic candidate selection remain unimplemented |
...
| Release packaging | Partial (strengthened) | The 0.35 development package is self-contained Windows x64, contains the engine/runtime/manifest and zero Python files. Packaging canonicalizes copied JSON/Markdown and the embedded schema to BOM-less UTF-8/LF and normalizes Windows packaging onto Windows PowerShell 5.1. The current exact-consolidation checkpoint produced the same 195-file, 84,386,430-byte tree and 35,992,947-byte ZIP in two local builds with SHA-256 `c7f68a994a185b80e211a751a05881537f0d836c53e195107cd4ae2fd4d11a76`; all five jobs in mandatory CI run `29936061264` passed and the hosted Windows artifact matched that local ZIP byte for byte. The preceding style-definition checkpoint matched mandatory CI run `29932018346` byte for byte at SHA-256 `ca713cbec08fa31dc67de8ef503bbf33c28a08e187980cc95d18093144c53f2f`; all five jobs passed. The earlier exact 0.35 live package, SHA-256 `e8f2e4b74fe65213197126c7aafb445452bd0e80bc05f7206d82672e4b09e59b`, passed the complete 48-action real-Word gate; the later saved-package slices do not change live COM code | Optional signing/provenance policy, published artifact and refreshed licensed Word gate before release |

## Current checkpoint evidence

- `dotnet test native/WordToolkit.Engine.Tests` — 274 passed.
- `dotnet test native/WordToolkit.Native.Tests` — 205 passed.
...
- The current development head exposes 14 public tools and 80 lazy actions. Two local
  builds produced the same 195-file, 84,359,203-byte tree and 35,982,900-byte ZIP with
  SHA-256
  `ca713cbec08fa31dc67de8ef503bbf33c28a08e187980cc95d18093144c53f2f`.
  Through the packaged executable, a 218-character two-command batch cloned the
  existing `Standard` style as `CodexDefinition` and server-selected six paragraphs for
  assignment without returning node IDs. The compact seven-operation plan line was
  2,146 characters. Candidate validation ran with no new SDK errors; apply matched the
  predicted fingerprint, changed only `word/styles.xml` and `word/document.xml`, retained
  a recovery backup, returned no XML, left the source fixture unchanged and did not alter
  the running Word process set. All five jobs in mandatory CI run `29932018346` passed;
  its hosted Windows ZIP matched the local archive byte for byte at the same size and
  SHA-256.
