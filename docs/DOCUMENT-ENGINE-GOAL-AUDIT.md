...
| Document dependency graph | Partial (operation-wide accounted boundary) | Deterministic `wddn_`/`wdde_` topology joins ten typed domains and preserves unresolved/external evidence. Compact compressed-row adjacency and the 128 MiB `wdg1` graph budget remain. `inspect_ooxml_dependencies` now creates one 640 MiB `wop1` lease before OPC retention and passes it through lossless XML, semantic fingerprints/nodes, styles, numbering, references, sections/settings, charts, figures/captions, content controls, tables and final graph construction. Package/XML charges precede guarded byte copies; semantic hashing is cancellable; table/figure/content-control aggregate byte checks reject the next parse. Success adds only a three-field operation tuple; typed `PACKAGE_LIMIT` details expose stage/attempted bytes without document data. The input schema is byte-for-byte unchanged | Shared immutable parsed-story storage, dense mixed-domain calibration, advanced DrawingML/VML layout, SmartArt, full OLE model, citations/bibliography sources, macros/signatures/encryption, co-authoring sessions, semantic query joins, incremental invalidation and mutation-impact policies |
...
| custom XML | Partial (typed read graph) | Relationship-discovered data/property items, GUID/schema/root metadata, malformed-store retention, built-in property stores, restricted binding targets and unified dependencies; values never leave the native inspector | Lossless value/schema edits, binding refresh, template planning, merge and broader Word-version proof |
...
| generate a dependency graph | Partial: one bounded graph now joins OPC, semantic containment, styles, numbering, references, sections, classic charts, logical figures/representations/resources, captions and selected/candidate associations, content controls, physical/built-in XML stores, binding targets, repeating-section items, nested tables and vertical-merge continuation cells with explicit coverage gaps; advanced drawing layout, SmartArt/full OLE, bibliography, active content and collaboration remain outside the typed spine |
...
| Requirement | State | Current evidence | Exit condition |
|---|---|---|---|
| Unit/regression tests | Partial | 438 engine, 283 native and 1309 Python tests pass at the current checkpoint, with 16 intentional Python skips | Coverage for every required feature and published failure corpus |
...
- `dotnet test native/WordToolkit.Engine.Tests` — 438 passed.
- `dotnet test native/WordToolkit.Native.Tests` — 283 passed.
