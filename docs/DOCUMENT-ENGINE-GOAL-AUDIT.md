...
| Capability negotiation | Partial (versioned discovery slice strengthened) | `get_wordtoolkit_capabilities` and the native `capabilities` CLI return one schema-versioned, hashed, paged contract for 87 actions without opening Word; semantic query, semantic-style plan/apply and comment-body plan/apply have explicit operation versions, output schemas, normalized permission and reversibility records in their inspected action contracts, while the closed v1 summary keeps its original shape; Word COM member capabilities remain separate | Migrate the remaining 82 actions and add a unified backend/document/feature matrix with runtime probes |
| Public SDK/CLI/MCP operation convergence | Partial (fifth vertical slice) | Inspection, transform, semantic query, semantic-style plan/apply and comment-body plan/apply own typed request/result/error contracts in `WordToolkit.Engine`. Comment edits share one strict JSON parser and one invariant-checked, schema-validated, atomic path across direct .NET, non-interactive JSON CLI and MCP without opening Word or returning comment text. Transform still provides replace-first-text, accept-all and reject-all and implements the neutral protocol-v1 benchmark adapter | Migrate remaining operations behind the same application boundary; generate language packages from canonical contracts and add a public rollback action rather than treating the backup path as a complete transaction API |
...
| Requirement | State | Current evidence | Exit condition |
|---|---|---|---|
| Unit/regression tests | Partial | 388 engine, 263 native and 1273 Python tests pass at the current checkpoint, with 16 intentional Python skips | Coverage for every required feature and published failure corpus |
...
| Release packaging | Partial (strengthened) | The 0.35 development package is self-contained Windows x64, contains the engine/runtime/manifest and zero Python files. Packaging canonicalizes copied JSON/Markdown and the embedded schema to BOM-less UTF-8/LF and normalizes Windows packaging onto Windows PowerShell 5.1. The current body-only comment checkpoint produced identical 196-file, 85,366,594-byte trees and 36,263,241-byte ZIPs in two local builds at SHA-256 `267ace7127d82c8f54c32f56682830770d6209f1880b4f00d38a31dc45aa5503`; expanded manifests matched at SHA-256 `25fd2c4afcfef292c95c5925d849cee55b5de1297dbbf0b05734e9edd9231373`. Packaged plan/apply on `mammoth_comments.docx` changed only `word/comments.xml`, reproduced the reviewed identities, preserved a byte-exact backup and started no Word process. Hosted CI run `29980881560` passed all five jobs; its downloaded Windows ZIP matched both local archives byte for byte and its expanded tree had zero differences. The preceding semantic-style interoperability checkpoint produced identical 196-file, 85,271,141-byte trees and 36,238,851-byte ZIPs in two local builds and mandatory CI run `29977852683` at SHA-256 `9a3fc1ae866ca9d5e1f9c1f40b21683491175b8e63125511f4dd6529bff6596e`; its downloaded hosted Windows ZIP matched both local archives byte for byte. The earlier exact 0.35 live package, SHA-256 `e8f2e4b74fe65213197126c7aafb445452bd0e80bc05f7206d82672e4b09e59b`, passed the complete 48-action real-Word gate; later saved-package slices do not change live COM code | Optional signing/provenance policy and refreshed licensed Word gate before release |

## Current checkpoint evidence

- `dotnet test native/WordToolkit.Engine.Tests` — 388 passed.
- `dotnet test native/WordToolkit.Native.Tests` — 263 passed.
