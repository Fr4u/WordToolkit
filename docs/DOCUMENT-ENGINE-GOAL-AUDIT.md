...
| Multi-command document transaction | Partial (text/review/package-patch slices, native plan/apply) | Bounded text edits and selective review decisions resolve against one snapshot and retain inverse payloads. The generic OPC patch slice creates deterministic portable add/replace/delete artifacts between two snapshots, recomputes semantic/risk evidence on apply, requires exact base/patch/apply-plan identities, then uses atomic persistence and a recovery backup | Broader heterogeneous semantic commands, permissions/approval policies and a unified validation profile across every command family |
| Optimistic concurrency | Partial (strengthened) | Forward plans require exact package fingerprints and part hashes; portable patch apply binds base/result fingerprints, patch ID and deterministic apply-plan ID, then the atomic writer rechecks destination content before and after candidate construction; live document versions exist | File identity/version integration, filesystem race/fault injection and Graph/Drive ETag support |
| Security policy | Partial (strengthened) | ZIP/XML bounds, DTD ban, external links never fetched, MCP redaction; patch archives reject unsafe/duplicate paths, unknown/duplicate manifest fields, hash/length drift, expansion bombs and unreferenced payloads. Apply has separate fail-closed authorizations for signature invalidation, macro/OLE/ActiveX, external relationships, opaque binaries and new errors; validation truncation/SDK-open failure is non-overridable | Protection enforcement, explicit signature verification/removal/resign workflow, encryption adapter, sandboxed backends and threat-model audit |
| Privacy/content-minimizing telemetry | Partial foundation | Text/review/patch plan metadata omit document text and payloads; review selection accepts redacted author fingerprints; patch operation hashes are opt-in; MCP plan/apply is stateless and retains no server-side document-content cache | Opt-in telemetry implementation, broader redaction tests, expiry and debug-bundle audit |
...
| AI planner | Partial foundation (strengthened) | Deterministic bounded text plans, typed review-decision plans and exact package-patch/apply plans report counts/impact without returning content. Patch plans expose semantic evidence, independently authorized risk classes, exact identities and baseline/candidate validation; lazy stateless apply requires reviewed IDs and snapshot fingerprint | Natural-language intent -> evidence -> broader heterogeneous typed plan -> cost/risk -> richer permissions and approval policy |
...
| Validator | Partial (strengthened) | OPC diagnostics plus exact serialized review and generic patch candidate reparse with bounded Microsoft Open XML SDK baseline/candidate comparison; new errors block by default, while validation truncation or SDK-open failure hard-blocks patch apply | Unified OPC/schema/extension/semantic/Word-open profiles and incremental validation |
...
| Semantic diff | Partial (strengthened initial slice) | Native two-layer saved-package comparison reports OPC entry changes separately from source-linked semantic differences across all projected Word stories. Matching ambiguity, fallbacks and unclassified changes remain explicit. A deterministic reversible OPC entry-payload patch can now be generated from the same exact snapshots and revalidated semantically at apply time | Richer typed property vocabulary, review-friendly rendering, semantic-command patch format, Word `CompareDocuments` adapter, benchmarked accuracy and cross-version corpus |
