...
| AI planner | Partial foundation (strengthened) | Deterministic bounded text plans and heterogeneous create/clone/assign style plans, typed review-decision plans, exact package-patch plans and three-way merge plans report counts/impact without returning content. Style plans can create or clone a definition and server-select its assignment targets in one reviewed atomic intent. Merge exposes stable conflict IDs, explicit three-choice resolutions, independently authorized risk classes, exact identities and baseline/candidate validation; lazy stateless apply requires reviewed IDs and all source fingerprints | Natural-language intent -> evidence -> broader heterogeneous typed plan -> cost/risk -> richer permissions and approval policy |
| Typed semantic mutations | Partial (text + style-definition/assignment + review transaction slices) | Text plans edit source-bound `w:t`, `w:delText` and `m:t`; `create_style` emits a minimal typed inherited definition, `clone_style` preserves an existing definition's modeled and opaque formatting under safe custom identity, and exact `set_style`/token-lean `set_style_where` assign existing or same-plan-created compatible styles. One plan joins `styles.xml` and selected document parts, validates the intermediate graph and final candidate, predicts one fingerprint and retains an exact inverse. Review plans accept/reject supported insertion/deletion/conflict wrappers, complete moves and property snapshots, plus numbering-change acceptance, inserted rows, cell-insertion acceptance and cell-deletion rejection. Unsupported or ambiguous structures fail closed | Arbitrary typed style formatting, rename/delete/consolidation/template alignment and `stylesWithEffects` mirroring; broader paragraph/run/table/field/math commands, remaining review vocabulary, permissions and durable recovery artifacts |
...
| `styles.xml` | Partial (typed read graph + create/clone/assignment mutation) | Defaults, latent metadata, four style types, inheritance diagnostics, theme integration and modeled effective formatting exist. Typed commands insert minimal inherited custom definitions or clone existing definitions with opaque extension preservation, then assign them in the same atomic transaction without reserializing prior style bytes | Arbitrary property authoring, safe rename/delete/consolidation, linked-style maintenance, `stylesWithEffects` mirroring, conditional table behavior and template alignment |
