...
| Comments/threaded comments/revisions | Partial (typed read graph + bounded mutation) | Source-linked comment/thread/person graph and authorship-linked revision graph remain redacted; saved-package plan/apply selectively accepts or rejects supported wrappers, complete moves and property snapshots and handles numbering-change acceptance, inserted rows, selected cell revisions and two proven-safe paragraph-mark merge shapes by revision ID or author fingerprint, with exact inverse and SDK candidate validation; live review remains available | Remaining paragraph/table/numbering/custom-XML transforms, full reaction/comment mutation, merge and accept/reject proof across Word versions |
...
| Capability negotiation | Partial (versioned discovery slice strengthened) | `get_wordtoolkit_capabilities` and the native `capabilities` CLI return one schema-versioned, hashed, paged contract for 85 actions without opening Word; semantic query plus semantic-style plan/apply now have explicit operation versions, output schemas, normalized permission and reversibility records in their inspected action contracts, while the closed v1 summary keeps its original shape; Word COM member capabilities remain separate | Migrate the remaining 82 actions and add a unified backend/document/feature matrix with runtime probes |
| Public SDK/CLI/MCP operation convergence | Partial (fourth vertical slice) | Inspection, transform, semantic query and the semantic-style plan/apply pair own typed request/result/error contracts in `WordToolkit.Engine`. Style operations now share seven high-level commands, a strict variant JSON parser, bounded selector expansion, exact intent/fingerprint preconditions, injected Microsoft schema validation and atomic backup across direct .NET, non-interactive JSON CLI and MCP without opening Word. Transform still provides replace-first-text, accept-all and reject-all and implements the neutral protocol-v1 benchmark adapter | Migrate remaining operations behind the same application boundary; generate language packages from canonical contracts and add a public rollback action rather than treating the backup path as a complete transaction API |
...
| Requirement | State | Current evidence | Exit condition |
|---|---|---|---|
| Unit/regression tests | Partial | 385 engine, 260 native and 1273 Python tests pass at the current checkpoint, with 16 intentional Python skips | Coverage for every required feature and published failure corpus |
...
| Release packaging | Partial (strengthened) | The 0.35 development package is self-contained Windows x64, contains the engine/runtime/manifest and zero Python files. Packaging canonicalizes copied JSON/Markdown and the embedded schema to BOM-less UTF-8/LF and normalizes Windows packaging onto Windows PowerShell 5.1. The current semantic-style interoperability checkpoint produced identical 196-file, 85,271,141-byte trees and 36,238,851-byte ZIPs in two local builds and mandatory CI run `29977852683` at SHA-256 `9a3fc1ae866ca9d5e1f9c1f40b21683491175b8e63125511f4dd6529bff6596e`; expanded manifests matched at SHA-256 `cd728f01ea4da97e303a1ad488ebd79d64b3b21113bc80aee8438cc29d074104`, the Open XML SDK adapter is present, all five jobs passed and the downloaded hosted Windows ZIP matched both local archives byte for byte. The preceding semantic-query checkpoint produced identical 195-file, 85,195,358-byte trees and 36,214,740-byte ZIPs in two local builds and mandatory CI run `29973644971` at SHA-256 `04af23655f262389c16a9b8eb94cc08d39173c5a9d80e74606929d26f9beb9e9`; the downloaded hosted Windows ZIP matched both local archives byte for byte, and the packaged executable returned five schema-valid matches without Word. The earlier exact 0.35 live package, SHA-256 `e8f2e4b74fe65213197126c7aafb445452bd0e80bc05f7206d82672e4b09e59b`, passed the complete 48-action real-Word gate; later saved-package slices do not change live COM code | Optional signing/provenance policy and refreshed licensed Word gate before release |

## Current checkpoint evidence

- `dotnet test native/WordToolkit.Engine.Tests` — 385 passed.
- `dotnet test native/WordToolkit.Native.Tests` — 260 passed.
