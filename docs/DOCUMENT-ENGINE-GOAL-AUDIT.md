...
| Requirement | State | Current evidence | Exit condition |
|---|---|---|---|
| Compact inspect | Implemented (strengthened initial slice) | `inspect_ooxml_package`, `inspect_ooxml_semantics`, lazy `query_ooxml_semantics`, `manage_ooxml_semantic_index`, `inspect_ooxml_sections`, `inspect_ooxml_styles`, `inspect_ooxml_numbering`, `inspect_ooxml_theme`, `inspect_ooxml_settings`, `inspect_ooxml_fonts`, `inspect_ooxml_references`, `inspect_ooxml_dependencies`, `inspect_ooxml_charts` and filtered `resolve_ooxml_formatting`; projected-part inventory, privacy redaction, fingerprints, exact filters, per-field/item bounds and offset paging; field-heavy TOC fixtures enforce sub-5000-character default reference/dependency responses and the chart fixture enforces a sub-5000-character default payload plus sub-8000-character complete JSON-RPC envelope | Opaque continuation tokens, remaining auxiliary/versioned parts and a representative cross-action token benchmark suite |
...
| Requirement | State | Current evidence | Exit condition |
|---|---|---|---|
| Unit/regression tests | Partial | 309 engine, 213 native and 1273 Python tests pass at the current checkpoint, with 16 intentional Python skips | Coverage for every required feature and published failure corpus |
...
- `dotnet test native/WordToolkit.Engine.Tests` — 309 passed.
- `dotnet test native/WordToolkit.Native.Tests` — 213 passed.
...
- `scripts/build_native_plugin.ps1` — self-contained native package built with no
  Python runtime.
- The current development head exposes 14 public tools and 81 lazy actions. Two local
  builds produced the same 195-file, 84,535,811-byte tree and 36,034,548-byte ZIP with
  SHA-256
  `26b7c1ff41933a26a2cd812aec70abe2f74408005eba8a76d48918409f2f0f88`.
  Through the packaged executable, the real LibreOffice chart fixture returned one
  plot, three series, two axes, 27 cached point entries and one embedded workbook in an
  875-character compact payload and 2,260-character complete response. It opened
  neither Word nor the workbook, followed no external target, returned no sensitive
  values and used no Python. Mandatory CI run `29947553481` passed all five jobs; its
  downloaded Windows artifact matched both local archives byte for byte.
