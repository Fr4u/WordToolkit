...
| Release packaging | Partial (strengthened) | The 0.35 development package is self-contained Windows x64, contains the engine/runtime/manifest and zero Python files. Packaging canonicalizes copied JSON/Markdown and the embedded schema to BOM-less UTF-8/LF and normalizes Windows packaging onto Windows PowerShell 5.1. The current semantic-query checkpoint produced identical 195-file, 85,195,358-byte trees and 36,214,740-byte ZIPs in two local builds at SHA-256 `04af23655f262389c16a9b8eb94cc08d39173c5a9d80e74606929d26f9beb9e9`; the packaged executable returned five schema-valid matches, discovered the query action and left the Word-process count at zero. The preceding transform checkpoint produced the same 195-file, 85,150,302-byte tree and 36,200,235-byte ZIP in two local builds and mandatory CI run `29970012359` at SHA-256 `e1ef21c763cae801f48bcfa43d1513ff279936a8fd026e00b28a04133755afe8`; the downloaded hosted Windows ZIP matched both local archives byte for byte. The earlier exact 0.35 live package, SHA-256 `e8f2e4b74fe65213197126c7aafb445452bd0e80bc05f7206d82672e4b09e59b`, passed the complete 48-action real-Word gate; later saved-package slices do not change live COM code | Publish and match the hosted query artifact, optional signing/provenance policy and refreshed licensed Word gate before release |
...
- The current development head exposes 15 public tools and 85 lazy actions. Inspection,
  ...
  the packaged executable returned the same schema-valid query result without Word.
  Transform also feeds the public benchmark protocol.
