...
| Typed semantic mutations | Partial (text + style + review transaction slices) | Text plans edit source-bound `w:t`, `w:delText` and `m:t`; exact `set_style` and token-lean `set_style_where` plans assign existing compatible resolvable paragraph/character/table styles to exact IDs or bounded server-resolved query matches; review plans accept/reject supported insertion/deletion/conflict wrappers, complete moves and property snapshots, plus numbering-change acceptance, inserted rows, cell-insertion acceptance and cell-deletion rejection. All preserve unrelated bytes, predict the result fingerprint and retain exact part-byte inverses; unsupported or ambiguous structures fail closed | Broader heterogeneous paragraph/run/table/field/math command set, style-definition lifecycle/template alignment, remaining review vocabulary, permissions and durable recovery artifacts |
...
|---|---|
| replace every definition with style `Definition` | Partial: one bounded `set_style_where` command can server-select paragraphs by strict text/property/structural evidence and assign an existing compatible `Definition` style under exact style/fingerprint/plan preconditions without echoing node IDs; linguistic classification of what constitutes a definition is not implemented |
...
| Requirement | State | Current evidence | Exit condition |
|---|---|---|---|
| Unit/regression tests | Partial | 270 engine, 203 native and 1273 Python tests pass at the current checkpoint, with 16 intentional Python skips | Coverage for every required feature and published failure corpus |
...
- The current development head exposes 14 public tools and 80 lazy actions. Two local
  builds produced the same 195-file, 84,327,130-byte tree and 35,973,436-byte ZIP with
  SHA-256
  `65ccaed09b608be6fb851a4ae38e54b4fc4ff9d1b7ac8b6f1e5aaa9d0dcbaeac`.
  Through the packaged executable, one `set_style_where` command selected six
  paragraphs in the Apache POI styles fixture and expanded them into six guarded style
  edits without returning node IDs to the model. Candidate validation ran with no new
  SDK errors; apply matched the predicted fingerprint, changed only
  `word/document.xml`, retained a recovery backup, returned no XML, left the source
  fixture unchanged and did not alter the running Word process set. The command input
  was 77.53% smaller than six exact commands; the 200-target regression fixture reduced
  command input from 19,014 to 186 characters (99.02%). All five jobs in mandatory CI
  run `29928388511` passed; its downloaded Windows ZIP matched both local archives
