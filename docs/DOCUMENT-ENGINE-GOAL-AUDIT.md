...
| Document dependency graph | Partial (strengthened cross-domain spine) | Deterministic `wddn_` nodes and `wdde_` edges join OPC reachability, semantic containment, explicit paragraph/run/table styles, style inheritance/link/defaults, numbering instances/abstracts/levels/picture bullets, field/bookmark targets, section header/footer bindings, classic charts, logical figures/representations/resources, caption candidates/associations, content controls/Custom XML bindings, nested tables and vertical-merge continuation cells. Missing/external targets and non-selected associations remain explicit; every endpoint, input fingerprint and resource budget is checked. Lazy inspection redacts keys/source by default and provides bounded impact traversal | Advanced DrawingML/VML layout, SmartArt, full OLE model, citations/bibliography sources, macros/signatures/encryption, co-authoring sessions, semantic query joins, incremental invalidation and mutation-impact policies |
...
| Requirement | State | Current evidence | Exit condition |
|---|---|---|---|
| Unit/regression tests | Partial | 408 engine, 272 native and 1279 Python tests pass at the current checkpoint, with 16 intentional Python skips | Coverage for every required feature and published failure corpus |
...
| Release packaging | Partial (strengthened) | The current 0.37 Windows x64 package is self-contained and contains zero Python files. Two repository-pinned .NET SDK 8.0.423 builds produced byte-identical 196-file, 85,719,250-byte trees and 36,369,889-byte ZIPs at SHA-256 `addb0ac796b9c5e41e6174f2bd1937d9fe996ae88015a81af5b000968caa63c7`; the enabled personal-plugin cache has zero file/hash differences and its CLI reports runtime 0.37.0, 90 actions and `inspect_ooxml_figures`. Packaging canonicalizes copied JSON/Markdown and the embedded schema to BOM-less UTF-8/LF and normalizes Windows packaging onto Windows PowerShell 5.1. The preceding 0.36 exact-target SVG checkpoint matched hosted artifact `8557592140` in CI run `29992366785`. An initial system-SDK package attempt was rejected before publication because exact SDK 8.0.423 was unavailable on PATH; the build was repeated with the pinned cached SDK rather than weakening `global.json`. The earlier exact 0.35 live package passed the complete 48-action real-Word gate; later saved-package slices do not change live COM code | Optional signing/provenance policy, a byte-budgeted dependency graph and refreshed licensed Word gate before release |

## Current checkpoint evidence

- `dotnet test native/WordToolkit.Engine.Tests` — 427 passed.
- `dotnet test native/WordToolkit.Native.Tests` — 280 passed.
