...
| Semantic HTML/SVG preview | Partial (initial HTML slice) | `wordtoolkit.render_ooxml_semantic_html/1.0` projects the main body or all text stories to deterministic self-contained HTML through one Engine/CLI/MCP path; links are inert, external resources and active content are never loaded, fields suppress instructions, revisions are annotated, equations are bounded linear-text fallbacks, unsupported drawings/extensions are visible placeholders, output is create-new and bounded to 256 MiB, and the result explicitly reports both `semantic_preview_non_paginated` and that the local artifact contains document content while the MCP response does not | Add source-linked object selection, SVG geometry, richer accessible math/list semantics, CSS/effective-format coverage, browser snapshots and shared cross-backend visual corpus without weakening the non-authoritative label |
...
| Requirement | State | Current evidence | Exit condition |
|---|---|---|---|
| Unit/regression tests | Partial | 393 engine, 267 native and 1273 Python tests pass at the current checkpoint, with 16 intentional Python skips | Coverage for every required feature and published failure corpus |
...
| Visual regression | Partial static semantic-HTML checkpoint | Packaged HTML parses into the expected story/paragraph tree, includes CSP and contains no external resource elements; the in-app browser blocked the local `file://` URL, so no browser screenshot or visual claim was fabricated | Versioned browser snapshots plus PDF/page/object baselines across rendering backends |
...
| Release packaging | Partial (strengthened) | The 0.35 development package is self-contained Windows x64, contains the engine/runtime/manifest and zero Python files. Packaging canonicalizes copied JSON/Markdown and the embedded schema to BOM-less UTF-8/LF and normalizes Windows packaging onto Windows PowerShell 5.1. The red-team-hardened semantic-HTML checkpoint produced identical 196-file, 85,424,288-byte trees and 36,281,322-byte ZIPs in two local builds at SHA-256 `8463738c94eeac9148d286ae71e0cdcefb96dcbe379a63956ceba664366a8882`; explicitly defined UTF-8/LF manifests of sorted relative path, byte length and file SHA-256 fields matched at SHA-256 `61ee297700a7a68d0eb825587b2cca9fdedf0c935ef0d96168034798f01d3c6e`. Hosted CI run `29984588930` passed all five jobs, and downloaded Windows artifact `8554525092` matched both local ZIPs and the normalized expanded tree exactly. The preceding body-only comment checkpoint produced identical 196-file, 85,366,594-byte trees and 36,263,241-byte ZIPs in two local builds at SHA-256 `267ace7127d82c8f54c32f56682830770d6209f1880b4f00d38a31dc45aa5503`; hosted CI run `29980881560` matched them. The preceding semantic-style checkpoint matched mandatory CI run `29977852683` at SHA-256 `9a3fc1ae866ca9d5e1f9c1f40b21683491175b8e63125511f4dd6529bff6596e`. The earlier exact 0.35 live package, SHA-256 `e8f2e4b74fe65213197126c7aafb445452bd0e80bc05f7206d82672e4b09e59b`, passed the complete 48-action real-Word gate; later saved-package slices do not change live COM code | Optional signing/provenance policy and refreshed licensed Word gate before release |

## Current checkpoint evidence

- `dotnet test native/WordToolkit.Engine.Tests` — 393 passed.
- `dotnet test native/WordToolkit.Native.Tests` — 267 passed.
...
- The current semantic-HTML checkpoint exposes
  ...
  supported package builds matched exactly at 196 files, 85,424,288 expanded bytes and
  ZIP SHA-256 `8463738c94eeac9148d286ae71e0cdcefb96dcbe379a63956ceba664366a8882`;
