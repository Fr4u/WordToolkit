...
| Requirement | State | Current evidence | Exit condition |
|---|---|---|---|
| Unit/regression tests | Partial | 274 engine, 205 native and 1273 Python tests pass at the current checkpoint, with 16 intentional Python skips | Coverage for every required feature and published failure corpus |
...
- The current development head exposes 14 public tools and 80 lazy actions. Two local
  builds produced the same 195-file, 84,409,303-byte tree and 35,998,031-byte ZIP with
  SHA-256
  `85919959a2176627a6a973ad61ccabe189eaf36fabc44c83c5eb53e7479a59f2`.
  Through the packaged executable, a clone first created one custom proof style from the
  Apache POI style fixture. An 83-character `rename_style` command produced a
  946-character compact plan with one positive byte delta, no new SDK errors and the
  same plan ID under reversed JSON property order. Apply matched its predicted
  fingerprint, changed only `word/styles.xml`, retained a byte-exact pre-apply backup,
  preserved the stable style ID plus every original non-style ZIP-entry payload and left
  the style graph with zero issues. It returned no XML, used no Python and did not open
  or alter Word. A packaged attempt to rename the default `Standard` style failed
  `UNSAFE_EDIT` without changing the file. All five jobs in mandatory CI run
  `29942107590` passed; its hosted Windows ZIP and expanded tree matched the local builds
  exactly at the same counts, sizes and SHA-256.
