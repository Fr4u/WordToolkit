...
| Document dependency graph | Partial (strengthened cross-domain spine and byte boundary) | Deterministic `wddn_` nodes and `wdde_` edges join OPC reachability, semantic containment, explicit paragraph/run/table styles, style inheritance/link/defaults, numbering instances/abstracts/levels/picture bullets, field/bookmark targets, section header/footer bindings, classic charts, logical figures/representations/resources, caption candidates/associations, content controls/Custom XML bindings, nested tables and vertical-merge continuation cells. Missing/external targets and non-selected associations remain explicit; every endpoint and input fingerprint is checked. A deterministic 128 MiB accounted-byte budget, 65,536-character metadata ceiling and compact incoming/outgoing compressed-row indexes now bound this graph's own retained construction. Five cold-process 99,997-node samples per version reduced median retained managed memory 16.8% and median dependency-build time 9.0%; median peak working set stayed flat within 0.1%. Lazy inspection redacts keys/source by default, exposes only a three-field byte budget and provides bounded impact traversal | Shared operation-wide resource lease across the upstream semantic/typed graphs, advanced DrawingML/VML layout, SmartArt, full OLE model, citations/bibliography sources, macros/signatures/encryption, co-authoring sessions, semantic query joins, incremental invalidation and mutation-impact policies |
...
| Requirement | State | Current evidence | Exit condition |
|---|---|---|---|
| Unit/regression tests | Partial | 429 engine, 281 native and 1279 Python tests pass at the current checkpoint, with 16 intentional Python skips | Coverage for every required feature and published failure corpus |
...
| Release packaging | Partial (strengthened) | The current 0.38 Windows x64 package is self-contained and contains zero Python files. Two repository-pinned .NET SDK 8.0.423 builds produced byte-identical 196-file, 85,730,528-byte trees and 36,372,963-byte ZIPs at SHA-256 `69c3406b238590dae096370a550ff6902352972cbe942b2344e2d80dca1e0541`; the personal marketplace and enabled plugin cache have zero path/length/hash differences and the installed CLI reports runtime 0.38.0, 90 actions and the updated dependency contract. A packaged smoke call returns the graph-local budget without opening Word or following external targets. The initial 0.38 registration succeeded through the installer; after the same-version schema-description refresh made installer backup collide with the active cache, the final tree was synchronized in place and verified file-for-file. Packaging canonicalizes copied JSON/Markdown and the embedded schema to BOM-less UTF-8/LF and normalizes Windows packaging onto Windows PowerShell 5.1. The preceding 0.37 and 0.36 checkpoints remain historical evidence. The earlier exact 0.35 live package passed the complete 48-action real-Word gate; later saved-package slices do not change live COM code | Optional signing/provenance policy, an operation-wide cross-graph resource lease and refreshed licensed Word gate before release |

## Current checkpoint evidence

- `dotnet test native/WordToolkit.Engine.Tests` — 429 passed.
- `dotnet test native/WordToolkit.Native.Tests` — 281 passed.
- `.venv/Scripts/python -m pytest -q` — 1279 passed, 16 intentionally skipped.
- The current dependency-memory checkpoint replaces eager per-node adjacency maps with
  two compact compressed-row indexes and applies a deterministic 128 MiB graph-local
  accounted-byte budget. The 99,997-node fixture accounts 130,132,744 bytes, uses
  1,599,952 adjacency-index bytes and rejects when rebuilt with a ceiling one byte lower.
  Five cold-process samples per version reduce median retained managed memory 16.8%,
  allocations 4.2%, dependency build time 9.0% and total measured time 5.4%. Median peak
  working set is effectively flat (+0.1%). This does not prove an
  operation-wide memory ceiling because every upstream typed graph is still built first.
  Two final package builds and the marketplace/cache trees match exactly at 196 files
  and ZIP SHA-256 `69c3406b238590dae096370a550ff6902352972cbe942b2344e2d80dca1e0541`;
  the installed runtime reports 0.38.0, 90 actions and the updated dependency description.
