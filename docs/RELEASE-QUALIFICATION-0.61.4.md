...
- Ambiguous guarded mutations previously ranked apply before plan by lexical tie-break.
  ...
  first, while an explicit `apply` token preserves apply-first intent. Six regression
  cases cover both directions, and the router blocks apply without an existing reviewed
  plan and exact bindings.
...
The exact final packaged candidate completed the full-live gate in 180.827 seconds:
...
- 194,859-byte Word PDF export passed;
- the test document was not left open and the runtime-owned Word process quit cleanly.

The first final-candidate gate attempt failed closed at guarded Undo because Word exposed a
...
- runtime assembly SHA-256:
  `ed3c98375cf74cf9587ed2e9337e631c2bf5441e56afca18f953254674f2b264`.
