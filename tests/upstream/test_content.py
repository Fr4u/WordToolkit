...
class TestAddList:
    ...
    def test_numbered_list(self):
        result = _j(server.add_list(["00000004"], style="numbered", start=1))
        assert result["paragraphs_updated"] == 1
        assert result["start"] == 1
        numbering_xml = _j(server.read_part("word/numbering.xml"))
        assert '<w:start w:val="1"' in numbering_xml["xml"]

    def test_numbered_list_preserves_explicit_zero_start(self):
        result = _j(server.add_list(["00000004"], style="numbered", start=0))
        assert result["start"] == 0
        numbering_xml = _j(server.read_part("word/numbering.xml"))
        assert '<w:start w:val="0"' in numbering_xml["xml"]
