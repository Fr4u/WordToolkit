...
class TestSetTrackChanges:
    ...
    def test_enable_sets_trackchanges_element(self):
        ...
        W = "{http://schemas.openxmlformats.org/wordprocessingml/2006/main}"
        tc = settings.find(f"{W}trackRevisions")
        assert tc is not None, "w:trackRevisions not found in settings.xml"
    ...
    def test_disable_removes_trackchanges_element(self):
        ...
        W = "{http://schemas.openxmlformats.org/wordprocessingml/2006/main}"
        tc = settings.find(f"{W}trackRevisions")
        assert tc is None, "w:trackRevisions should be absent after disabling"
    ...
    def test_enable_twice_is_idempotent(self):
        """Enabling twice should not duplicate w:trackRevisions."""
        server.set_track_changes(enabled=True)
        server.set_track_changes(enabled=True)
        doc = server._doc
        settings = doc._tree("word/settings.xml")
        W = "{http://schemas.openxmlformats.org/wordprocessingml/2006/main}"
        elements = settings.findall(f"{W}trackRevisions")
        assert len(elements) == 1, "Should not have duplicate w:trackRevisions"
...
class TestSetTrackChangesCreatesSettings:
    ...
    def test_set_track_changes_creates_settings_if_absent(self, tmp_path: Path):
        ...
        tc = settings.find(f"{W_NS}trackRevisions")
        assert tc is not None, "w:trackRevisions must be present after enabling"
