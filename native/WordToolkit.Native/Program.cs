...
internal static class Program
{
    private const string Usage =
        "usage: wordtoolkit-native [capabilities [--schema | [--query <text>] [--offset <n>] [--limit <n>]] [--format json] | extensions [--query <text>] [--offset <n>] [--limit <1..32>] [--format json] | audit-log verify <path> [--max-bytes <n>] [--max-events <n>] [--format json] | inspect-package <path> [--include-details] [--max-items <1..200>] [--format json] | inspect-encryption <path> [--format json] | query-package --request <query.json|-> [--format json] | render-package --request <request.json|-> [--backend semantic-html|semantic-svg] [--format json] | style-package --mode <plan|apply> --request <request.json|-> [--format json] | numbering-repair-package --mode <plan|apply> --request <request.json|-> [--format json] | relationship-repair-package --mode <inspect|plan|apply> --request <request.json|-> [--format json] | comment-body-package --mode <plan|apply> --request <request.json|-> [--format json] | patch-rollback-package --mode <plan|apply> --request <request.json|-> [--format json] | transform-package <input> <output> --operation <name> [--find-text <text> --replace-text <text>] [--format json] | flat-opc-package <input> <output> --direction <to_flat_opc|from_flat_opc> [--format json] | docx-platform-adapter --protocol-version 1 --operation <operation.json> --input <input.docx> --output <output.docx> | --create-test-document <path> | --benchmark-active-word]";
