...
internal static class HeadingOutlinePackageCli
{
    private const string Usage =
        "usage: wordtoolkit-native heading-outline-package --request <request.json|-> [--format json]";
    ...
    )
    {
        ...
        try
        {
            ...
            var request = HeadingOutlineOperationJson.ParseInspectRequest(json);
            var result = new HeadingOutlineWordPackageOperation().Inspect(request);
        ...
        )
        {
            WriteError(error, new WordToolkitOperationError(
                "NOT_FOUND",
                "The requested heading-outline JSON file does not exist",
                null,
                Retryable: false
            ));
            return 66;
        ...
        )
        {
            WriteError(error, new WordToolkitOperationError(
                "INVALID_INPUT",
                "The heading-outline JSON path is invalid",
                null,
                Retryable: false
            ));
            return 64;
        ...
        catch (UnauthorizedAccessException)
        {
            WriteError(error, new WordToolkitOperationError(
                "ACCESS_DENIED",
                "The heading-outline JSON file cannot be read",
                null,
                Retryable: false
            ));
            return 77;
        ...
        catch (IOException)
        {
            WriteError(error, new WordToolkitOperationError(
                "IO_ERROR",
                "The heading-outline JSON could not be read",
                null,
                Retryable: true
            ));
            return 74;
        ...
        catch (Exception)
        {
            WriteError(error, new WordToolkitOperationError(
                "INTERNAL_ERROR",
                "The heading-outline operation failed",
                null,
                Retryable: false
            ));
            return 70;
    ...
    private static string ReadRequestFile(string requestPath)
    {
        ...
        if (stream.Length > HeadingOutlineWordPackageContract.MaximumRequestJsonCharacters * 4L)
        {
    ...
    private static string ReadBounded(TextReader reader)
    {
        ...
        while (true)
        {
            ...
            if (result.Length + read > HeadingOutlineWordPackageContract.MaximumRequestJsonCharacters)
            {
                throw new WordToolkitOperationException(
                    "INVALID_INPUT",
                    $"Request JSON cannot exceed {HeadingOutlineWordPackageContract.MaximumRequestJsonCharacters} characters"
    ...
    private static int UsageError(TextWriter error)
    {
        WriteError(error, new WordToolkitOperationError(
            "INVALID_INPUT",
            "Invalid heading-outline-package arguments",
            Usage,
            Retryable: false
        ));
        return 64;
