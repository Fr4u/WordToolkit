...
internal static class NotePackageCli
{
    private const string Usage =
        "usage: wordtoolkit-native note-package --mode <inspect|plan|apply> --request <request.json|-> [--format json]";
    ...
    )
    {
        ...
        try
        {
            ...
            {
                "inspect" => operation.Inspect(NoteOperationJson.ParseInspectRequest(json)),
                "plan" => operation.Plan(NoteOperationJson.ParsePlanRequest(json)),
                _ => operation.Apply(NoteOperationJson.ParseApplyRequest(json)),
            };
        ...
        )
        {
            WriteError(error, new WordToolkitOperationError(
                "NOT_FOUND",
                "The requested note operation JSON file does not exist",
                null,
                Retryable: false
            ));
            return 66;
        ...
        )
        {
            WriteError(error, new WordToolkitOperationError(
                "INVALID_INPUT",
                "The note operation JSON path is invalid",
                null,
                Retryable: false
            ));
            return 64;
        ...
        catch (UnauthorizedAccessException)
        {
            WriteError(error, new WordToolkitOperationError(
                "ACCESS_DENIED",
                "The note operation JSON file cannot be read",
                null,
                Retryable: false
            ));
            return 77;
        ...
        catch (IOException)
        {
            WriteError(error, new WordToolkitOperationError(
                "IO_ERROR",
                "The note operation JSON could not be read",
                null,
                Retryable: true
            ));
            return 74;
    ...
    private static string ReadRequestFile(string requestPath)
    {
        ...
        if (stream.Length > NoteWordPackageContract.MaximumRequestJsonCharacters * 4L)
        {
    ...
    private static string ReadBounded(TextReader reader)
    {
        ...
        while (true)
        {
            ...
            if (result.Length + read > NoteWordPackageContract.MaximumRequestJsonCharacters)
            {
                throw new WordToolkitOperationException(
                    "INVALID_INPUT",
                    $"Request JSON cannot exceed {NoteWordPackageContract.MaximumRequestJsonCharacters} characters"
    ...
    private static int UsageError(TextWriter error)
    {
        WriteError(error, new WordToolkitOperationError(
            "INVALID_INPUT",
            "Invalid note-package arguments",
            Usage,
            Retryable: false
        ));
        return 64;
