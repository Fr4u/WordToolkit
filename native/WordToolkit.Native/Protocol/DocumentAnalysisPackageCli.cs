...
internal static class DocumentAnalysisPackageCli
{
    private const string Usage =
        "usage: wordtoolkit-native analyze-package --request <request.json|-> [--format json]";
    ...
    )
    {
        ...
        try
        {
            ...
            var request = DocumentAnalysisOperationJson.ParseRequest(json);
            var result = new DocumentAnalysisWordPackageOperation().Analyze(request);
        ...
        )
        {
            WriteError(error, new WordToolkitOperationError(
                "NOT_FOUND",
                "The requested document-analysis JSON file does not exist",
                null,
                Retryable: false
            ));
            return 66;
        ...
        )
        {
            WriteError(error, new WordToolkitOperationError(
                "INVALID_INPUT",
                "The document-analysis JSON path is invalid",
                null,
                Retryable: false
            ));
            return 64;
        ...
        catch (UnauthorizedAccessException)
        {
            WriteError(error, new WordToolkitOperationError(
                "ACCESS_DENIED",
                "The document-analysis JSON file cannot be read",
                null,
                Retryable: false
            ));
            return 77;
        ...
        catch (IOException)
        {
            WriteError(error, new WordToolkitOperationError(
                "IO_ERROR",
                "The document-analysis JSON could not be read",
                null,
                Retryable: true
            ));
            return 74;
        ...
        catch (Exception)
        {
            WriteError(error, new WordToolkitOperationError(
                "INTERNAL_ERROR",
                "The document-analysis operation failed",
                null,
                Retryable: false
            ));
            return 70;
    ...
    private static string ReadRequestFile(string requestPath)
    {
        ...
        if (stream.Length > DocumentAnalysisWordPackageContract.MaximumRequestJsonCharacters * 4L)
        {
    ...
    private static string ReadBounded(TextReader reader)
    {
        ...
        while (true)
        {
            ...
            if (result.Length + read
                > DocumentAnalysisWordPackageContract.MaximumRequestJsonCharacters)
            {
                throw new WordToolkitOperationException(
                    "INVALID_INPUT",
                    $"Request JSON cannot exceed {DocumentAnalysisWordPackageContract.MaximumRequestJsonCharacters} characters"
    ...
    private static int UsageError(TextWriter error)
    {
        WriteError(error, new WordToolkitOperationError(
            "INVALID_INPUT",
            "Invalid analyze-package arguments",
            Usage,
            Retryable: false
        ));
        return 64;
