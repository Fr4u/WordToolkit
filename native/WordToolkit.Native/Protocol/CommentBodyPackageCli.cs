...
internal static class CommentBodyPackageCli
{
    private const string Usage =
        "usage: wordtoolkit-native comment-body-package --mode <plan|apply> --request <request.json|-> [--format json]";
    ...
    )
    {
        ...
        try
        {
            ...
            object result = mode == "plan"
                ...
                : operation.Apply(CommentBodyEditOperationJson.ParseApplyRequest(json));
        ...
        )
        {
            WriteError(error, new WordToolkitOperationError(
                "NOT_FOUND",
                "The requested comment body operation JSON file does not exist",
                null,
                Retryable: false
            ));
            return 66;
        ...
        )
        {
            WriteError(error, new WordToolkitOperationError(
                "INVALID_INPUT",
                "The comment body operation JSON path is invalid",
                null,
                Retryable: false
            ));
            return 64;
        ...
        catch (UnauthorizedAccessException)
        {
            WriteError(error, new WordToolkitOperationError(
                "ACCESS_DENIED",
                "The comment body operation JSON file cannot be read with current permissions",
                null,
                Retryable: false
            ));
            return 77;
        ...
        catch (IOException)
        {
            WriteError(error, new WordToolkitOperationError(
                "IO_ERROR",
                "The comment body operation JSON could not be read",
                null,
                Retryable: true
            ));
            return 74;
    ...
    private static string ReadRequestFile(string requestPath)
    {
        ...
        if (
            stream.Length
            > CommentBodyWordPackageContract.MaximumRequestJsonCharacters * 4L
        )
        {
    ...
    private static string ReadBounded(TextReader reader)
    {
        ...
        while (true)
        {
            ...
            if (
                result.Length + read
                > CommentBodyWordPackageContract.MaximumRequestJsonCharacters
            )
            {
                throw new WordToolkitOperationException(
                    "INVALID_INPUT",
    ...
    {
        "INVALID_INPUT" => 64,
        "NOT_FOUND" or "COMMENT_NOT_FOUND" => 66,
        "INVALID_PACKAGE" or "INVALID_WORD_PACKAGE" or "PACKAGE_LIMIT" => 65,
        "VERSION_CONFLICT" or "PLAN_MISMATCH" or "MATCH_COUNT_MISMATCH"
            or "TRANSACTION_LIMIT" => 75,
        "ACCESS_DENIED" => 77,
        "IO_ERROR" => 74,
    ...
    private static int UsageError(TextWriter error)
    {
        WriteError(error, new WordToolkitOperationError(
            "INVALID_INPUT",
            "Invalid comment-body-package arguments",
            Usage,
            Retryable: false
        ));
        return 64;
