...
internal static class NumberingRebuildPackageCli
{
    private const string Usage =
        "usage: wordtoolkit-native numbering-rebuild-package --mode <inspect|plan|apply> --request <request.json|-> [--format json]";
    ...
    )
    {
        ...
        try
        {
            ...
            object result = mode switch
            {
                "inspect" => operation.Inspect(
                    NumberingRebuildOperationJson.ParseInspectRequest(json)
                ),
                "plan" => operation.Plan(
                    NumberingRebuildOperationJson.ParsePlanRequest(json)
                ),
                _ => operation.Apply(
                    NumberingRebuildOperationJson.ParseApplyRequest(json)
                ),
            };
        ...
        )
        {
            WriteError(error, new WordToolkitOperationError(
                "NOT_FOUND",
                "The requested numbering rebuild JSON file does not exist",
                null,
                Retryable: false
            ));
            return 66;
        ...
        )
        {
            WriteError(error, new WordToolkitOperationError(
                "INVALID_INPUT",
                "The numbering rebuild JSON path is invalid",
                null,
                Retryable: false
            ));
            return 64;
        ...
        catch (UnauthorizedAccessException)
        {
            WriteError(error, new WordToolkitOperationError(
                "ACCESS_DENIED",
                "The numbering rebuild JSON file cannot be read",
                null,
                Retryable: false
            ));
            return 77;
        ...
        catch (IOException)
        {
            WriteError(error, new WordToolkitOperationError(
                "IO_ERROR",
                "The numbering rebuild JSON could not be read",
                null,
                Retryable: true
            ));
            return 74;
    ...
    private static string ReadRequestFile(string requestPath)
    {
        ...
        if (stream.Length
            > NumberingRebuildWordPackageContract.MaximumRequestJsonCharacters * 4L)
        {
    ...
    private static string ReadBounded(TextReader reader)
    {
        ...
        while (true)
        {
            ...
            if (result.Length + read
                > NumberingRebuildWordPackageContract.MaximumRequestJsonCharacters)
            {
                throw new WordToolkitOperationException(
                    "INVALID_INPUT",
                    $"Request JSON cannot exceed {NumberingRebuildWordPackageContract.MaximumRequestJsonCharacters} characters"
    ...
    private static int UsageError(TextWriter error)
    {
        WriteError(error, new WordToolkitOperationError(
            "INVALID_INPUT",
            "Invalid numbering-rebuild-package arguments",
            Usage,
            Retryable: false
        ));
        return 64;
