...
internal static class NumberingRepairPackageCli
{
    private const string Usage =
        "usage: wordtoolkit-native numbering-repair-package --mode <plan|apply> --request <request.json|-> [--format json]";
    ...
    )
    {
        ...
        try
        {
            ...
            object result = mode == "plan"
                ? operation.Plan(NumberingRepairOperationJson.ParsePlanRequest(json))
                : operation.Apply(NumberingRepairOperationJson.ParseApplyRequest(json));
        ...
        )
        {
            WriteError(error, new WordToolkitOperationError(
                "NOT_FOUND",
                "The requested numbering repair JSON file does not exist",
                null,
                Retryable: false
            ));
            return 66;
        ...
        )
        {
            WriteError(error, new WordToolkitOperationError(
                "INVALID_INPUT",
                "The numbering repair JSON path is invalid",
                null,
                Retryable: false
            ));
            return 64;
        ...
        catch (UnauthorizedAccessException)
        {
            WriteError(error, new WordToolkitOperationError(
                "ACCESS_DENIED",
                "The numbering repair JSON file cannot be read",
                null,
                Retryable: false
            ));
            return 77;
        ...
        catch (IOException)
        {
            WriteError(error, new WordToolkitOperationError(
                "IO_ERROR",
                "The numbering repair JSON could not be read",
                null,
                Retryable: true
            ));
            return 74;
    ...
    private static string ReadRequestFile(string requestPath)
    {
        ...
        if (stream.Length > NumberingRepairWordPackageContract.MaximumRequestJsonCharacters * 4L)
        {
    ...
    private static string ReadBounded(TextReader reader)
    {
        ...
        while (true)
        {
            ...
            if (result.Length + read
                > NumberingRepairWordPackageContract.MaximumRequestJsonCharacters)
            {
                throw new WordToolkitOperationException(
                    "INVALID_INPUT",
                    $"Request JSON cannot exceed {NumberingRepairWordPackageContract.MaximumRequestJsonCharacters} characters"
    ...
    private static int UsageError(TextWriter error)
    {
        WriteError(error, new WordToolkitOperationError(
            "INVALID_INPUT",
            "Invalid numbering-repair-package arguments",
            Usage,
            Retryable: false
        ));
        return 64;
