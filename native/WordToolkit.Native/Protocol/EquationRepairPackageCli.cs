...
internal static class EquationRepairPackageCli
{
    private const string Usage =
        "usage: wordtoolkit-native equation-repair-package --mode <inspect|plan|apply> --request <request.json|-> [--format json]";
    ...
    )
    {
        ...
        try
        {
            ...
            object result = mode switch
            {
                "inspect" => operation.Inspect(
                    EquationRepairOperationJson.ParseInspectRequest(json)
                ),
                "plan" => operation.Plan(
                    EquationRepairOperationJson.ParsePlanRequest(json)
                ),
                _ => operation.Apply(
                    EquationRepairOperationJson.ParseApplyRequest(json)
                ),
            };
        ...
        )
        {
            WriteError(error, new WordToolkitOperationError(
                "NOT_FOUND",
                "The requested OfficeMath repair JSON file does not exist",
                null,
                Retryable: false
            ));
            return 66;
        ...
        )
        {
            WriteError(error, new WordToolkitOperationError(
                "INVALID_INPUT",
                "The OfficeMath repair JSON path is invalid",
                null,
                Retryable: false
            ));
            return 64;
        ...
        catch (UnauthorizedAccessException)
        {
            WriteError(error, new WordToolkitOperationError(
                "ACCESS_DENIED",
                "The OfficeMath repair JSON file cannot be read",
                null,
                Retryable: false
            ));
            return 77;
        ...
        catch (IOException)
        {
            WriteError(error, new WordToolkitOperationError(
                "IO_ERROR",
                "The OfficeMath repair JSON could not be read",
                null,
                Retryable: true
            ));
            return 74;
        ...
        catch (Exception)
        {
            WriteError(error, new WordToolkitOperationError(
                "INTERNAL_ERROR",
                "The OfficeMath repair package operation failed",
                null,
                Retryable: false
            ));
            return 70;
    ...
    private static string ReadRequestFile(string requestPath)
    {
        ...
        if (stream.Length > EquationRepairWordPackageContract.MaximumRequestJsonCharacters * 4L)
        {
    ...
    private static string ReadBounded(TextReader reader)
    {
        ...
        while (true)
        {
            ...
            if (result.Length + read
                > EquationRepairWordPackageContract.MaximumRequestJsonCharacters)
            {
                throw new WordToolkitOperationException(
                    "INVALID_INPUT",
                    $"Request JSON cannot exceed {EquationRepairWordPackageContract.MaximumRequestJsonCharacters} characters"
    ...
    private static int UsageError(TextWriter error)
    {
        WriteError(error, new WordToolkitOperationError(
            "INVALID_INPUT",
            "Invalid equation-repair-package arguments",
            Usage,
            Retryable: false
        ));
        return 64;
