...
internal static class TemplateStyleAlignmentPackageCli
{
    private const string Usage =
        "usage: wordtoolkit-native template-style-alignment-package --mode <inspect|plan|apply> --request <request.json|-> [--format json]";
    ...
    )
    {
        ...
        try
        {
            ...
            object result = mode switch
            {
                "inspect" => operation.Inspect(
                    TemplateStyleAlignmentOperationJson.ParseInspectRequest(json)
                ),
                "plan" => operation.Plan(
                    TemplateStyleAlignmentOperationJson.ParsePlanRequest(json)
                ),
                _ => operation.Apply(
                    TemplateStyleAlignmentOperationJson.ParseApplyRequest(json)
                ),
            };
        ...
        )
        {
            WriteError(error, new WordToolkitOperationError(
                "NOT_FOUND",
                "The requested template style-alignment JSON file does not exist",
                null,
                Retryable: false
            ));
            return 66;
        ...
        )
        {
            WriteError(error, new WordToolkitOperationError(
                "INVALID_INPUT",
                "The template style-alignment JSON path is invalid",
                null,
                Retryable: false
            ));
            return 64;
        ...
        catch (UnauthorizedAccessException)
        {
            WriteError(error, new WordToolkitOperationError(
                "ACCESS_DENIED",
                "The template style-alignment JSON file cannot be read",
                null,
                Retryable: false
            ));
            return 77;
        ...
        catch (IOException)
        {
            WriteError(error, new WordToolkitOperationError(
                "IO_ERROR",
                "The template style-alignment JSON could not be read",
                null,
                Retryable: true
            ));
            return 74;
        ...
        catch (Exception)
        {
            WriteError(error, new WordToolkitOperationError(
                "INTERNAL_ERROR",
                "The template style-alignment operation failed",
                null,
                Retryable: false
            ));
            return 70;
    ...
    private static string ReadRequestFile(string requestPath)
    {
        ...
        if (stream.Length
            > TemplateStyleAlignmentWordPackageContract.MaximumRequestJsonCharacters * 4L)
        {
            throw new WordToolkitOperationException(
                "INVALID_INPUT",
                $"Request JSON cannot exceed {TemplateStyleAlignmentWordPackageContract.MaximumRequestJsonCharacters} characters"
            );
        }
    ...
    private static string ReadBounded(TextReader reader)
    {
        ...
        while (true)
        {
            ...
            if (result.Length + read
                > TemplateStyleAlignmentWordPackageContract.MaximumRequestJsonCharacters)
            {
                throw new WordToolkitOperationException(
                    "INVALID_INPUT",
                    $"Request JSON cannot exceed {TemplateStyleAlignmentWordPackageContract.MaximumRequestJsonCharacters} characters"
    ...
    private static int UsageError(TextWriter error)
    {
        WriteError(error, new WordToolkitOperationError(
            "INVALID_INPUT",
            "Invalid template-style-alignment-package arguments",
            Usage,
            Retryable: false
        ));
        return 64;
