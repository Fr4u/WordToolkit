...
internal static class PatchRollbackPackageCli
{
    private const string Usage =
        "usage: wordtoolkit-native patch-rollback-package --mode <plan|apply> --request <request.json|-> [--format json]";
    ...
    )
    {
        ...
        for (var index = 0; index < arguments.Count; index++)
        {
            ...
            if (
                ...
                && mode is null
                && candidateMode is "plan" or "apply"
            )
            {
                mode = candidateMode;
                continue;
        ...
        try
        {
            ...
            object result = mode == "plan"
                ...
                : operation.Apply(PatchRollbackOperationJson.ParseApplyRequest(json));
        ...
        )
        {
            WriteError(
                ...
                new WordToolkitOperationError(
                    "NOT_FOUND",
                    "The requested patch rollback JSON file does not exist",
                    null,
                    Retryable: false
                )
            );
        ...
        )
        {
            WriteError(
                ...
                new WordToolkitOperationError(
                    "INVALID_INPUT",
                    "The patch rollback JSON path is invalid",
                    null,
                    Retryable: false
                )
            );
        ...
        catch (UnauthorizedAccessException)
        {
            WriteError(
                ...
                new WordToolkitOperationError(
                    "ACCESS_DENIED",
                    "The patch rollback JSON file cannot be read with current permissions",
                    null,
                    Retryable: false
                )
            );
        ...
        catch (IOException)
        {
            WriteError(
                ...
                new WordToolkitOperationError(
                    "IO_ERROR",
                    "The patch rollback JSON could not be read",
                    null,
                    Retryable: true
                )
            );
    ...
    private static string ReadRequestFile(string requestPath)
    {
        ...
        if (
            stream.Length
            > PatchRollbackWordPackageContract.MaximumRequestJsonCharacters * 4L
        )
        {
    ...
    private static string ReadBounded(TextReader reader)
    {
        ...
        while (true)
        {
            ...
            if (
                result.Length + read
                > PatchRollbackWordPackageContract.MaximumRequestJsonCharacters
            )
            {
                throw new WordToolkitOperationException(
                    "INVALID_INPUT",
    ...
    internal static int ExitCode(string code) => code switch
    {
        ...
        "INVALID_PACKAGE" or "INVALID_WORD_PACKAGE" or "PACKAGE_LIMIT"
            or "PATCH_LIMIT" or "DIFF_LIMIT" => 65,
        "VERSION_CONFLICT" or "PLAN_MISMATCH" or "SOURCE_CHANGED" => 75,
        "ACCESS_DENIED" => 77,
        "IO_ERROR" => 74,
        "PATCH_POLICY_BLOCKED" or "VALIDATION_FAILED" or "RESULT_MISMATCH"
            or "INVALID_PATCH" or "RECOVERY_REQUIRED" => 78,
        _ => 70,
    };
    ...
    private static int UsageError(TextWriter error)
    {
        WriteError(
            ...
            new WordToolkitOperationError(
                "INVALID_INPUT",
                "Invalid patch-rollback-package arguments",
                Usage,
                Retryable: false
            )
        );
