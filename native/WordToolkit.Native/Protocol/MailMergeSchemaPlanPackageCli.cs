...
internal static class MailMergeSchemaPlanPackageCli
{
    private const string Usage =
        "usage: wordtoolkit-native mail-merge-schema-package --request <request.json|-> [--format json]";
    ...
    )
    {
        ...
        try
        {
            ...
            var request = MailMergeSchemaPlanOperationJson.ParseRequest(json);
            var result = new MailMergeSchemaPlanWordPackageOperation().Plan(request);
        ...
        )
        {
            WriteError(error, new WordToolkitOperationError(
                "NOT_FOUND",
                "The requested mail-merge schema JSON file does not exist",
                null,
                Retryable: false
            ));
            return 66;
        ...
        )
        {
            WriteError(error, new WordToolkitOperationError(
                "INVALID_INPUT",
                "The mail-merge schema JSON path is invalid",
                null,
                Retryable: false
            ));
            return 64;
        ...
        catch (UnauthorizedAccessException)
        {
            WriteError(error, new WordToolkitOperationError(
                "ACCESS_DENIED",
                "The mail-merge schema JSON file cannot be read",
                null,
                Retryable: false
            ));
            return 77;
        ...
        catch (IOException)
        {
            WriteError(error, new WordToolkitOperationError(
                "IO_ERROR",
                "The mail-merge schema JSON could not be read",
                null,
                Retryable: true
            ));
            return 74;
        ...
        catch (Exception)
        {
            WriteError(error, new WordToolkitOperationError(
                "INTERNAL_ERROR",
                "The mail-merge schema operation failed",
                null,
                Retryable: false
            ));
            return 70;
    ...
    private static string ReadRequestFile(string requestPath)
    {
        ...
        if (stream.Length > MailMergeSchemaPlanWordPackageContract.MaximumRequestJsonCharacters * 4L)
        {
    ...
    private static string ReadBounded(TextReader reader)
    {
        ...
        while (true)
        {
            ...
            if (result.Length + read
                > MailMergeSchemaPlanWordPackageContract.MaximumRequestJsonCharacters)
            {
                throw new WordToolkitOperationException(
                    "INVALID_INPUT",
                    $"Request JSON cannot exceed {MailMergeSchemaPlanWordPackageContract.MaximumRequestJsonCharacters} characters"
    ...
    private static int UsageError(TextWriter error)
    {
        WriteError(error, new WordToolkitOperationError(
            "INVALID_INPUT",
            "Invalid mail-merge-schema-package arguments",
            Usage,
            Retryable: false
        ));
        return 64;
