...
internal static class StylePackageCli
{
    private const string Usage =
        "usage: wordtoolkit-native style-package --mode <plan|apply> --request <request.json|-> [--format json]";
    ...
    )
    {
        ...
        try
        {
            ...
            object result = mode == "plan"
                ...
                : operation.Apply(StyleEditOperationJson.ParseApplyRequest(json));
        ...
        )
        {
            WriteError(
                ...
                new WordToolkitOperationError(
                    "NOT_FOUND",
                    "The requested style operation JSON file does not exist",
                    null,
                    Retryable: false
                )
            );
        ...
        )
        {
            WriteError(
                ...
                new WordToolkitOperationError(
                    "INVALID_INPUT",
                    "The style operation JSON path is invalid",
                    null,
                    Retryable: false
                )
            );
        ...
        catch (UnauthorizedAccessException)
        {
            WriteError(
                ...
                new WordToolkitOperationError(
                    "ACCESS_DENIED",
                    "The style operation JSON file cannot be read with current permissions",
                    null,
                    Retryable: false
                )
            );
        ...
        catch (IOException)
        {
            WriteError(
                ...
                new WordToolkitOperationError(
                    "IO_ERROR",
                    "The style operation JSON could not be read",
                    null,
                    Retryable: true
                )
            );
    ...
    private static string ReadRequestFile(string requestPath)
    {
        ...
        if (
            stream.Length
            > StyleWordPackageContract.MaximumRequestJsonCharacters * 4L
        )
        {
    ...
    private static string ReadBounded(TextReader reader)
    {
        ...
        while (true)
        {
            ...
            if (
                result.Length + read
                > StyleWordPackageContract.MaximumRequestJsonCharacters
            )
            {
                throw new WordToolkitOperationException(
                    "INVALID_INPUT",
    ...
    private static int ExitCode(string code) => code switch
    {
        ...
        "VERSION_CONFLICT" or "PLAN_MISMATCH" or "EMPTY_SELECTION"
            or "SELECTION_LIMIT" or "TRANSACTION_LIMIT" => 75,
        "ACCESS_DENIED" => 77,
        "IO_ERROR" => 74,
        "SIGNED_PACKAGE" or "VALIDATOR_REQUIRED" or "OOXML_SCHEMA_INVALID"
            or "VALIDATION_FAILED" or "UNSAFE_EDIT" or "RECOVERY_REQUIRED" => 78,
        _ => 70,
    };
    ...
    private static int UsageError(TextWriter error)
    {
        WriteError(
            ...
            new WordToolkitOperationError(
                "INVALID_INPUT",
                "Invalid style-package arguments",
                Usage,
                Retryable: false
            )
        );
