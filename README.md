...
The runtime implements 48 tested Word Live actions plus 27 standalone,
...
byte spans. Suppressions are explicit and bounded. Fix metadata is advisory and reports
`implemented=false`; the linter does not repair or mutate a package. Its response keeps
execution completeness separate from whole-document coverage, so unmodeled domains
...
Run the complete packaged 14-tool/75-action live acceptance gate:
