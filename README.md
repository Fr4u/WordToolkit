...
OPC digital-signature integrity is a separate bounded preflight:

```powershell
wordtoolkit-native inspect-signatures .\signed.docx --view signatures --format json
```

The shared `wordtoolkit.inspect_ooxml_signatures/1.0` contract verifies the signature
origin graph, supported XMLDSIG signature values, signed package-part digests and OPC
Relationship Transform subsets without opening Word or using the network. It returns no
document content, raw XML, certificate bytes, signer identity or local path. Embedded
certificates are verification keys only: chain trust and revocation remain explicitly
false. See
[`docs/RESEARCH-OPC-DIGITAL-SIGNATURES-2026.md`](docs/RESEARCH-OPC-DIGITAL-SIGNATURES-2026.md).
...
The development manifest/runtime is the locally qualified20 0.60.1 candidate (`0.60.1+codex.20260821194458`). Read-only installation inspection passed: artifact/source/cache parity is 199 files and 91,105,061 expanded bytes; cache path `C:\Users\Admin\.codex\plugins\cache\personal\wordtoolkit\0.60.1+codex.20260821194458`, and runtime guidance 149/149. Q20 inherits q17 live-full evidence (59/59, 15/149, combined 5/5, Word 0); its delta is limited to OCR trust lock/recovery hardening and validation-snapshot cleanup. The latest immutable public release remains
