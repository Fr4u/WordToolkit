...
def _append_picture(
    ...
    *,
    ...
    """Append a schema-complete pic:pic payload to a:graphicData element."""
    pic = etree.SubElement(parent, f"{_PIC}pic")
    non_visual = etree.SubElement(pic, f"{_PIC}nvPicPr")
    c_nv_pr = etree.SubElement(non_visual, f"{_PIC}cNvPr")
    c_nv_pr.set("id", str(drawing_id))
    c_nv_pr.set("name", f"Image {drawing_id}")
    etree.SubElement(non_visual, f"{_PIC}cNvPicPr")
    ...
    shape = etree.SubElement(pic, f"{_PIC}spPr")
    transform = etree.SubElement(shape, f"{A}xfrm")
    offset = etree.SubElement(transform, f"{A}off")
    offset.set("x", "0")
    offset.set("y", "0")
    extent = etree.SubElement(transform, f"{A}ext")
    extent.set("cx", str(width_emu))
    extent.set("cy", str(height_emu))
    geometry = etree.SubElement(shape, f"{A}prstGeom")
...
class ImagesMixin:
    ...
    def insert_image(
        ...
        *,
        ...
        cnv_frame = etree.SubElement(inline, f"{WP}cNvGraphicFramePr")
    ...
    def set_image_alt_text(self, rId: str, alt_text: str, title: str = "") -> dict:
        ...
        if doc_pr is None:
            ...
            doc_pr.set("id", str(drawing_id))
            doc_pr.set("name", f"Image {drawing_id}")
            # wp:inline and wp:anchor are sequence-sensitive.  docPr belongs
            # after the extent/effect/wrap elements and before the drawing
            # graphic frame properties / graphic payload.
            insert_at = len(container)
            for index, child in enumerate(container):
                if child.tag in {f"{WP}cNvGraphicFramePr", f"{A}graphic"}:
                    insert_at = index
                    break
            container.insert(insert_at, doc_pr)
