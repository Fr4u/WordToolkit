...
def _get_analyzer():
    ...
    The model must be installed in the immutable container image.  WordToolkit
    never downloads executable/model content while handling a user request.
    """
    global _analyzer
    ...
    try:
        ...
        raise RuntimeError(
            f"Required PII model {_MODEL_NAME!r} is not installed; "
            "install it in the container image before enabling PII tools"
        ) from exc
