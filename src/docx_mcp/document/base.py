...
class BaseMixin:
    ...
    def open(self, *, package_path: str | Path | None = None) -> dict:
        ...
        # WordToolkit vendors this engine and confines every extraction to the
        # owning session directory.  The upstream default used the process-wide
        # temporary directory, which is unsuitable for a multi-user HTTP service.
        session_tmp = self.source_path.parent / ".work"
        ...
        xml_files = ["[Content_Types].xml", "_rels/.rels"]
        # Core properties
        ...
        for rel_path in xml_files:
            ...
            if full_path.exists():
                try:
                    # Reject DTD/entity declarations instead of merely declining
                    # to resolve them.  This closes the raw-entity and quadratic
                    # expansion paths before a part reaches any document mixin.
                    head = full_path.read_bytes()[:4096].upper()
                    if b"<!DOCTYPE" in head or b"<!ENTITY" in head:
                        raise DocxMcpError(
                            ErrCode.OOXML_INVALID,
                            f"Unsafe XML declaration in {rel_path}",
                        )
        ...
        with zipfile.ZipFile(output, "w", zipfile.ZIP_DEFLATED) as zf:
            for root, _dirs, files in os.walk(self.workdir):
                for fname in files:
                    ...
                    arcname = fpath.relative_to(self.workdir).as_posix()
