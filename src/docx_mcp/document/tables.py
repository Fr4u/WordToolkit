...
class TablesMixin:
    ...
    def add_table(
        ...
        *,
        ...
        for _ in range(rows):
            ...
            # OOXML tracks an inserted table through inserted rows. A bare
            # w:ins inside tblPr is not valid CT_TblPrBase content.
    ...
    def set_header_row(self, table_index: int) -> dict:
        ...
        if tr_pr.find(f"{W}tblHeader") is None:
            header = etree.Element(f"{W}tblHeader")
            revision_tags = {f"{W}ins", f"{W}del", f"{W}trPrChange"}
            insert_at = next(
                (
                    index
                    for index, child in enumerate(tr_pr)
                    if child.tag in revision_tags
                ),
                len(tr_pr),
            )
            tr_pr.insert(insert_at, header)
    ...
    def set_column_widths(self, table_index: int, widths_cm: list[float]) -> dict:
        ...
        # Make geometry deterministic in Word and LibreOffice.  Auto/percentage
        # widths routinely drift during round trips, especially in landscape
        # sections and after merged cells are introduced.
        tbl_pr = tbl.find(f"{W}tblPr")
        if tbl_pr is None:
            tbl_pr = etree.Element(f"{W}tblPr")
            tbl.insert(0, tbl_pr)

        def upsert_property(local: str, before: set[str]) -> etree._Element:
            existing = tbl_pr.find(f"{W}{local}")
            if existing is not None:
                return existing
            element = etree.Element(f"{W}{local}")
            for index, child in enumerate(tbl_pr):
                if child.tag in {f"{W}{name}" for name in before}:
                    tbl_pr.insert(index, element)
                    return element
            tbl_pr.append(element)
            return element

        table_width = upsert_property(
            "tblW", {"tblInd", "tblBorders", "tblLayout", "tblCellMar", "ins", "del"}
        )
        table_width.set(f"{W}type", "dxa")
        table_width.set(f"{W}w", str(sum(twips)))
        table_indent = upsert_property(
            "tblInd", {"tblBorders", "tblLayout", "tblCellMar", "ins", "del"}
        )
        table_indent.set(f"{W}type", "dxa")
        table_indent.set(f"{W}w", "120")
        layout = upsert_property("tblLayout", {"tblCellMar", "ins", "del"})
        layout.set(f"{W}type", "fixed")
        margins = upsert_property("tblCellMar", {"ins", "del"})
        for side, value in (("top", 80), ("left", 120), ("bottom", 80), ("right", 120)):
            item = margins.find(f"{W}{side}")
            if item is None:
                item = etree.SubElement(margins, f"{W}{side}")
            item.set(f"{W}type", "dxa")
            item.set(f"{W}w", str(value))
        ...
        for row_el in tbl.findall(f"{W}tr"):
            ...
            for tc in cells:
                ...
                if tc_w is None:
                    tc_w = etree.Element(f"{W}tcW")
                    tc_pr.insert(0, tc_w)
                tc_w.set(f"{W}type", "dxa")
                tc_w.set(f"{W}w", str(sum(twips[grid_position : grid_position + span])))
                grid_position += span
    ...
    def set_table_borders(
        ...
        borders = etree.Element(f"{W}tblBorders")
        before = {
            f"{W}shd",
            f"{W}tblLayout",
            f"{W}tblCellMar",
        ...
        insert_at = next(
            (index for index, child in enumerate(tbl_pr) if child.tag in before),
            len(tbl_pr),
        )
        tbl_pr.insert(insert_at, borders)
