...
from docx_mcp.document.base import CT, RELS, W14, WP, A, R, W, _preserve
from docx_mcp.document.images import _append_picture
...
class MarkdownConverter:
    ...
    def _run(self, text: str) -> None:
        """Parse and render."""
        self._consolidate_body_section_properties()
    ...
    def _consolidate_body_section_properties(self) -> None:
        """Keep one final body sectPr while preserving non-conflicting settings."""
        sections = self._body.findall(f"{W}sectPr")
        if len(sections) < 2:
            return
        final = sections[-1]
        order = (
        ...
        for earlier in sections[:-1]:
            for child in list(earlier):
                local = etree.QName(child).localname
                if local in {"headerReference", "footerReference"}:
                    duplicate = any(
                        item.tag == child.tag and item.get(f"{W}type") == child.get(f"{W}type")
                        for item in final
                    )
                else:
                    duplicate = final.find(child.tag) is not None
                if duplicate:
                    continue
                earlier.remove(child)
                wanted = rank.get(local, len(order))
                insert_at = len(final)
                for index, item in enumerate(final):
                    if rank.get(etree.QName(item).localname, len(order)) > wanted:
                        insert_at = index
                        break
                final.insert(insert_at, child)
            self._body.remove(earlier)
    ...
    def _render_image(self, parent: etree._Element, token: dict) -> None:
        ...
        if src.startswith(("http://", "https://")):
            ...
            if not img_path.exists():
                ...
                return
            self._embed_image(parent, str(img_path), alt)

    def _embed_image(self, parent: etree._Element, image_path: str, alt: str = "") -> None:
        ...
        # Schema-complete inline DrawingML image.
        width_emu = 2_000_000
        height_emu = 2_000_000
        document = self._doc._require("word/document.xml")
        drawing_id = self._doc._next_drawing_id(document)
        ...
        locks = etree.SubElement(frame, f"{A}graphicFrameLocks")
